...
public class PortalsCommand {
        ...
        dispatcher.register(
                literal("portals").requires(source -> source.hasPermissionLevel(2))
                        .then(literal("nether_portal")
                                .then(literal("enable").executes(context -> portal(context, NETHER_PORTAL, true)))
                                .then(literal("disable").executes(context -> portal(context, NETHER_PORTAL, false)))
                        )
                        .then(literal("end_portal")
                                .then(literal("enable").executes(context -> portal(context, END_PORTAL, true)))
                                .then(literal("disable").executes(context -> portal(context, END_PORTAL, false)))
                        )
                        .then(literal("end_gateways")
                                .then(literal("enable").executes(context -> portal(context, END_GATEWAYS, true)))
                                .then(literal("disable").executes(context -> portal(context, END_GATEWAYS, false)))
                        )
                        .then(literal("eden_ring_portal")
                                .then(literal("enable").executes(context -> portal(context, EDEN_RING_PORTAL, true)))
                                .then(literal("disable").executes(context -> portal(context, EDEN_RING_PORTAL, false)))
                        )
    ...
    private static int portal(CommandContext<ServerCommandSource> context, int type, boolean enabled) {
        switch (type) {
